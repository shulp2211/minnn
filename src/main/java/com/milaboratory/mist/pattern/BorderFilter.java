...
public final class BorderFilter implements Filter {
    ...
    public BorderFilter(boolean leftSide, NucleotideSequence seq) {
        this(leftSide, seq, seq.size(), true);
    }

    public BorderFilter(boolean leftSide, NucleotideSequence seq, int minNucleotides) {
        this(leftSide, seq, minNucleotides, true);
    }

    public BorderFilter(boolean leftSide, NucleotideSequence seq, boolean useTarget) {
        this(leftSide, seq, seq.size(), useTarget);
    }
     ...
     */
    public BorderFilter(boolean leftSide, NucleotideSequence seq, int minNucleotides, boolean useTarget) {
    ...
    public Match checkMatch(Match match) {
        ...
        for (int motifSize = minNucleotides; motifSize <= seq.size(); motifSize++) {
            ...
            if (new FuzzyMatchPattern(motif).match(targetEdge).isFound())
                return match;
        }
        return null;
    }
